...
export default function getResult(a, b, op) {
  ...
  switch (op) {
    ...
    case "x2":
      res = Math.pow(a, 2);
      res = aTailLength
        ? Math.round(res * Math.pow(10, aTailLength * 2)) /
          Math.pow(10, aTailLength * 2)
        : res;
      break;
    case "x3":
      res = Math.pow(a, 3);
      res = aTailLength
        ? Math.round(res * Math.pow(10, aTailLength * 9)) /
          Math.pow(10, aTailLength * 9)
        : res;
      break;
    case "1/x":
      res = a === 0 ? "Error" : 1 / a;
  ...
  if (res !== "Error") {
    res = String(res);
    let resTailLength = res.includes(".")
      ? res.slice(res.indexOf(".") + 1).length
      : 0;
